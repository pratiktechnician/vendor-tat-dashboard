...
for (let i = 1; i < v3Main.length; i++) {
  ...
  let tatVal = parseFloat(r[8]);
  const remarks = r[9] || '';

  // Clean TAT value if it is negative or a huge error number
  ...
  const recordObj = {
    ...
    status: status || (doneDate ? 'Fully Completed' : 'Pending'),
    remarks
  };

  riplRecords.push(recordObj);

  if (status.toLowerCase().includes('complete') || doneDate !== '') riplCompleted++;
...
for (let i = 1; i < v4Main.length; i++) {
  ...
  let tatVal = parseFloat(r[11]);
  let targetVal = parseFloat(r[13]);
  const remarks = r[16] || '';
  ...
  if (status.toLowerCase().includes('complete') || doneDate !== '') malfonicCompleted++;
...
// Generate the final merged dashboard json
const mergedDashboard = {
  ripl: {
    ...
    wip: riplWip,
    pending: riplRecords.length - riplCompleted - riplWip,
  ...
  malfonic: {
    ...
    wip: malfonicWip,
    pending: malfonicRecords.length - malfonicCompleted - malfonicWip,
...
fs.writeFileSync('new_vendors_analysis_clean.json', JSON.stringify(mergedDashboard, null, 2));
console.log('Saved cleaned JSON datasets.');
